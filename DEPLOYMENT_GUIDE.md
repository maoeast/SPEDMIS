...
### ✅ 代码质量验证
- [x] 全量 Jest 测试通过（`test/` 下 38 个测试文件）
- [x] 所有模块编译通过
- [x] 代码安全审计完成
...
# 输出文件：
# - dist/特殊教育多模态干预系统 Setup 1.2.2.exe (NSIS 安装程序)
# - dist/builder-debug.yml / latest.yml (构建产物)
```
...
3. **功能完整性测试**
   ```
   - 产品信息显示：中文名+英文名+版权信息
   - 模块列表加载：应显示 6 大领域 + 可选模块（AI 心理测评 / IEP）
...
2. **发布说明**
   ```
   版本 1.2.x 更新内容：
   - AI 教师工作台（智能体聊天、多接入点、知识技能、只读工具、图片视觉）
   - AI 心理测评入口（psyseen 登录状态同步）
   - 7 大模块自定义命名、系统维护可见性开关
   - 流式回复空闲超时（长回复不再被 60s 总超时中断）
   ```
...
- Node.js：16.0.0+
- 发布日期：2026-07-29
- 安全修复：P0 x4, P1 x8（1.0.0 时代基线，已全部修复）
