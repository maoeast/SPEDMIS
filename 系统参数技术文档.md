...
激活文件是激活码的分发载体，由激活码生成工具导出（CLI `--export-lis` / GUI「导出激活文件」），用户可在激活页面「导入激活文件(.lis)」自动激活。
...
**解析规则**：忽略空行与 `#` 注释行；字段 key 大小写不敏感、顺序无关；兼容 BOM 与 CRLF/LF 换行；机器码/激活码必须为 64 位十六进制。

**导入流程**：激活页面选择 .lis 文件 → 渲染层读取文本 → IPC `parse-activation-lis` 交给主进程解析（`modules/activation-lis.js`）→ 校验文件内机器码与当前机器一致 → 自动填入激活码并提交激活。

**相关文件**：`modules/activation-lis.js`（构建/解析单一实现）、`config.js`（IPC 通道）、`tools/activation-tool-cli.js` / `tools/activation-tool-gui.html`（导出）。
...
- 独立 `BrowserWindow` + 最小化预加载脚本 `ai-preload.js`（contextBridge），渲染层不直接访问数据库、文件系统、网络 Provider 或明文密钥
- AI 数据独立存储于 `%APPDATA%/特殊教育多模态干预系统/ai-assistant.db`（sql.js，schema v5），不污染激活数据与使用统计库
- 领域逻辑全部位于 `modules/ai-*.js`，Main 进程统一承载数据库、密钥、Provider、提示词拼装与流式调度

#### 数据表（schema v5）

| 表 | 职责 |
| --- | --- |
| ai_agent | 智能体（内置/自定义、启停状态、技能绑定、工具开关 tools_enabled） |
| ai_conversation | 会话 |
| ai_message | 消息（含知识快照与 provenance 溯源列） |
| ai_skill / ai_agent_skill | 知识技能与智能体绑定（content_version 版本门控幂等升级） |
| ai_token_ledger | Token 账本与月度额度 |
| ai_provider | 服务接入点（endpoints_json 多接入点/模型 ID 列表、supports_vision 视觉能力标记） |
| ai_preference | 偏好设置（knowledge_section_visible 为休眠字段） |
| ai_attachment | 图片附件（受控存储，删会话回收孤儿） |
| ai_tool_call | 只读工具调用审计 |

#### 密钥与安全

- API Key 经 Electron `safeStorage` 加密持久化，无明文降级，仅 Main 进程解密使用；不进入 HTML、日志、普通配置或数据库明文字段
- 首次向外部模型发送内容前必须完成隐私告知确认；日志与工具结果脱敏

#### Provider 与请求

- 支持 DeepSeek / 火山方舟的 OpenAI 兼容接口；多接入点列表（endpoints_json），活动接入点即 model，旧单 model 字段兼容回退
- 流式响应采用**空闲超时**：响应头到达与每段数据到达时重置计时，模型持续产出不被 60s 总超时误杀，仅"两段数据间隔 >60s"触发超时；一次性响应（completeChat）保持总超时
- 图片视觉按 provider `supports_vision` 门控（默认关闭，本地拦截）；附件全量校验（扩展名/MIME/魔数/尺寸/大小）+ sha256

#### 治理与工具

- 知识技能按智能体绑定精确注入，设置字符/技能/引用数硬上限 + 结构化截断标记
- 只读工具（查询干预应用目录、查询聚合使用统计）：按 agent 开关启用、Main 执行、有界非流式循环、逐工具超时、结果脱敏 + 字段投影（剥离路径）
- 系统维护开关：`advanced-settings.html` → `modules/ai-feature-flags-manager.js` → `ai-feature-flags.json`，控制智能体管理 / 知识技能 / 本月额度面板可见性，AI 面板只读反映

#### 相关文件

- 主进程：`modules/ai-database.js`、`ai-secret-store.js`、`ai-provider-client.js`、`ai-service.js`、`ai-ipc.js`、`ai-agent-catalog.js`、`ai-skill-catalog.json`、`ai-tool-registry.js`、`ai-tool-validator.js`、`ai-attachment-store.js`、`ai-feature-flags-manager.js`
- 渲染层：`ai-assistant.html/js/css`、`ai-preload.js`
...
| 版本号 | 日期 | 主要变更 |
|-------|------|---------|
| v1.2.2 | 2026-07-29 | 新增 AI 教师工作台（Phase 1-5）、AI 心理测评、模块自定义命名、系统维护开关 |
